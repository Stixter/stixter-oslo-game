...
namespace Stixter.Plexi.ScreenManager.Handlers
{
    ...
    {
        public static void PlaceItem(int lightCount, List<Platform> platforms, PickUpItem pickUpItem)
        {
            var array = new byte[8];
            var random1 = new Random();
            random1.NextBytes(array);
            var random = array[lightCount] % platforms.Count;
            var valueForItemPlacement = 200;
            if (platforms[random].GetLength() < 5)
            {
                valueForItemPlacement = 100;
            }
            var onPlatform = array[lightCount] % valueForItemPlacement;
